...
di = create_directory(os.path.join(dirname, 'outputs', 'inversion', '1d', 'synthetic'))
...
fname = os.path.join(di, 'optimisation_progress_continuous_{:s}')
if use_regularisation:
...
fname = os.path.join(plot_dir, 'optimisation_progress_continuous')
if use_regularisation:
    fname = '_'.join([fname, 'reg'])
...
if not plot_only:
    ...
    for i in range(len(gauges), N*N):
        axes[i//N, i % N].axis(False)
    plt.tight_layout()
    plt.savefig(os.path.join(plot_di, 'timeseries_optimised_continuous_{:d}.pdf'.format(level)))
