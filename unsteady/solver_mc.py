...
class AdaptiveProblem(AdaptiveProblemBase):
    ...
    def create_intermediary_spaces(self):
        ...
        if self.op.wetting_and_drying:
            spaces = [P1 for P1 in self.P1]
            self.intermediary_solutions_bathymetry = [Function(space) for space in spaces]
            self.intermediary_solutions_alpha = [Function(space) for space in spaces]
        if self.op.solve_tracer:
    ...
    def set_fields(self, init=False):
        ...
        if self.op.solve_exner:
            if init:
                ...
                self.wetting_and_drying_alpha = [self.op.set_wetting_and_drying_alpha(bathymetry, self.P1[i]) for i, bathymetry in enumerate(self.fwd_solutions_bathymetry)]
            if not init and self.iteration == 0:
                self.wetting_and_drying_alpha = [self.op.set_wetting_and_drying_alpha(bathymetry, self.P1[i]) for i, bathymetry in enumerate(self.fwd_solutions_bathymetry)]
            for i, bathymetry in enumerate(self.fwd_solutions_bathymetry):
                self.depth[i] = DepthExpression(
                    ...
                    wetting_and_drying_alpha=self.wetting_and_drying_alpha[i],
                )
            ...
            if init:
                ...
                self.wetting_and_drying_alpha = [self.op.set_wetting_and_drying_alpha(bathymetry, self.P1[i]) for i, bathymetry in enumerate(self.bathymetry)]
            if not init and self.iteration == 0:
                self.wetting_and_drying_alpha = [self.op.set_wetting_and_drying_alpha(bathymetry, self.P1[i]) for i, bathymetry in enumerate(self.bathymetry)]
            for i, bathymetry in enumerate(self.bathymetry):
                self.depth[i] = DepthExpression(
    ...
    def project_to_intermediary_mesh(self, i):
        ...
        if self.op.wetting_and_drying:
            self.intermediary_solutions_bathymetry[i].project(self.bathymetry[i])
            self.intermediary_solutions_alpha[i].project(self.depth[i].wetting_and_drying_alpha)
        if self.op.solve_tracer:
    ...
    def copy_data_from_intermediary_mesh(self, i):
        ...
        if self.op.wetting_and_drying:
            print('copy')
            self.bathymetry[i].dat.data[:] = self.intermediary_solutions_bathymetry[i].dat.data
            self.depth[i].wetting_and_drying_alpha.dat.data[:] = self.intermediary_solutions_alpha[i].dat.data
        if self.op.solve_tracer:
