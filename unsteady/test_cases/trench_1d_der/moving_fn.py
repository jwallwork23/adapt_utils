...
def moving(res, alpha, beta, gamma, filename):
    ...
    f.write("alpha = {:.1f}".format(alpha))
    f.write("beta = {:.1f}".format(beta))
    f.write("gamma = {:.1f}".format(gamma))
    ...
    f.write("Discretisation error: {:.4e}".format(np.sqrt(sum([(df['bath'][i] - df_real['bath'][i])**2 for i in range(len(df_real))]))))
    f.write('\n')
    f.close()
